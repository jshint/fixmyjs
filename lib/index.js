...
(function (name, definition) {
  ...
  function getRules(overrides) {
    ...
    var filter = fu.curry(fu.filter, function (ruleName) {
      return options[ruleName] === true && rules.hasOwnProperty(ruleName)
    })
    var map = fu.curry(fu.map, function (ruleName) {
      return rules[ruleName]
    })

    return fu.foldl(function (v, f) {
      return f(v)
    }, [filter, map], Object.keys(options))
  }
